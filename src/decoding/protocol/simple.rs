...
impl SimpleDecodingProtocol {
    ...
    fn read_unsigned(&self, source: &mut dyn BitSource, num_bits: usize) -> Result<u128,DecodeError> {
        let mut bits = vec![false; num_bits];
        source.read(&mut bits).map_err(|read| DecodeError::Reading(read))?;
    ...
    fn read_signed(&self, source: &mut dyn BitSource, num_bits: usize) -> Result<i128,DecodeError> {
